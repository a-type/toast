...
export interface LinkerExternalParams {
  ttl?: string;
  dsc?: string;
  att?: string;
  src?: string;
  ...
  mode?: 'postMessage' | 'direct';
}

export interface RecipeLinkerProps {
  externalParams: LinkerExternalParams;
  onDone(recipe: any): void;
}
interface InternalRecipeLinkerProps extends RecipeLinkerProps {
  link(recipeData: any): Promise<any>;
  setImage(
    recipeId: string,
    recipeImage: string,
...
class RecipeLinker extends React.PureComponent<InternalRecipeLinkerProps> {
...
export default compose<InternalRecipeLinkerProps, RecipeLinkerProps>(
  graphql(SetImage, {
    props: ({ mutate }) => ({
